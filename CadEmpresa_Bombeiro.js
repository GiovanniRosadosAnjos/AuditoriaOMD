...
// Rev90: integração pontual com CRUD.js + limpeza controlada do CRUD antigo como fallback comentado.
// Não altera Licença Sanitária, AFE, Contatos ou Saúde.
...
function mapBombeiroFromSupabase(row = {}) {
  return {
    id: row.id || '',
    ...
    status: row.status || calcularStatusBombeiro(row.validade || ''),
...
function mapBombeiroToSupabase(dados = {}, empresaId) {
  return {
    empresa_id: empresaId,
    tipo_licenca: dados.tipo_licenca || '',
    numero: dados.numero || '',
    validade: dados.validade || null,
    status: dados.status || calcularStatusBombeiro(dados.validade || '') || null,
...
function preencherCampos(dados = {}) {
  ...
  const status = dados.status || calcularStatusBombeiro(validade);
...
export async function buscarBombeiroPorEmpresa(empresaId) {
  ...
  const registros = await buscarRegistros(TABELA_BOMBEIRO, {
    filtros: [{ campo: 'empresa_id', valor: empresaId }],
    orderBy: 'created_at',
    ascending: false,
    limit: 1,
    mapper: mapBombeiroFromSupabase
  });
...
export async function salvarBombeiroPorEmpresa(empresaId) {
  ...
  const payload = mapBombeiroToSupabase(dados, empresaId);

  const salvo = await salvarRegistro(TABELA_BOMBEIRO, payload, {
    id: existente?.id || '',
    mapper: mapBombeiroFromSupabase
  });

  await carregarBombeiroPorEmpresa(empresaId);
  return salvo;
}
...
/*
================================================================================
FALLBACK / CÓDIGO ANTIGO DE BOMBEIRO — NÃO EXECUTAR
Mantido temporariamente apenas para consulta e rollback manual.
O fluxo ativo da Rev90 usa CRUD.js.
